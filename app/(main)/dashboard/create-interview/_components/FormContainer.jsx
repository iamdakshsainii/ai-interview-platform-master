...
const FormContainer = ({onHandleInputChange}) => {
  ...
  return (
    <div className='p-5 bg-white rounded-2xl'>
       <div>
          <h2 className='text-sm font-medium'>Job Positions</h2>
          <Input placeholder='e.g Full Stack Developer'
          className='mt-2' onChange={(event) => onHandleInputChange('jobPosition',event.target.value)}/>
       </div>
       ...
       <div>
          ...
          <div className='flex gap-3 flex-wrap mt-2'>
            {InterviewType.map((type, index) =>(
                <div key={index} className= {`flex gap-2 item-center cursor-pointer p-1 px-4
                 bg-white border border-grey-300 rounded-2xl
